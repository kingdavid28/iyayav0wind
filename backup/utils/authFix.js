...
export const fixAuthIssues = async () => {
  try {
    logger.info('🔧 Fixing auth issues...');
    ...
    logger.info('✅ Auth data cleared successfully - please login again');
    return true;
  } catch (error) {
    logger.error('❌ Failed to fix auth issues:', error);
    return false;
  }
};

// Auto-fix auth issues on app start if needed
export const autoFixAuthOnStartup = async () => {
  try {
    const token = await AsyncStorage.getItem('auth_token');
    if (token && token.includes('undefined') || token === 'null') {
      logger.warn('Detected corrupted auth token, auto-fixing...');
      return await fixAuthIssues();
    }
    return true;
  } catch (error) {
    logger.error('Auto-fix auth check failed:', error);
    return false;
  }
};
